...
BIN_NAME=qos-data-store

VERSION := $(shell grep "const Version " version.go | sed -E 's/.*"(.+)"$$/\1/')
GIT_COMMIT=$(shell git rev-parse HEAD)
GIT_DIRTY=$(shell test -n "`git status --porcelain`" && echo "+CHANGES" || true)
IMAGE_NAME := "hyperpilot/qos-data-store"
...
build-docker:
	@echo "building ${BIN_NAME} ${VERSION}"
	docker build -t qos-data-store:build -f Dockerfile.build .
	docker run --name=qos-data-store -v $(GOPATH):/gopath/  qos-data-store:build
	docker rm -f qos-data-store
